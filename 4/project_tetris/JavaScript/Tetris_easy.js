    ...
    function DrawBoard() {
        ...
        for (let r = 0; r < row; r++) {
            for (let c = 0; c < col; c++) {
                ...
                cell.style.boxSizing = "border-box";    // 테두리도 칸 크기에 포함해도록
                cell.style.backgroundColor = board[r][c] === 0 ? "#292828" : board[r][c];   // 칸 색깔 정하기 ===0이 참이면 #292828 불이면 board[r][c]색 사용
    ...
    function ClearBlock() {
        ...
        for (let r = row - 1; r >= 0; r--) { // 아래줄 부터 체크를 하기위해 보드 아래쪽 row-1 부터 위쪽으로
            if (board[r].every(cell => cell !== 0)) {   // 한줄이 모두 1인가 확인
                ...
                board.unshift(Array(col).fill(0)); //  맨위 빈줄 추가, 배열 구조상 위쪽에 새로운 요소를 넣음 이미 있는 배열들은 밀리기에 자동으로 블록들은 내려가짐
                linesCleared++; // 지운줄 갯수
